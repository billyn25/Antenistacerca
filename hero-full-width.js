...
// La fotografía aprobada se conserva completa, incluidos camiseta y furgoneta.
export const HERO = '/assets/hero-antenista-panorama.webp';
export const HERO_SHA256 = 'e8b6a8429c1c9f8314779b6226da28568260a839cc3290d7eb4efd4d6a0faa48';
export const WIDTH = 1672;
export const HEIGHT = 941;
const CSS_PATH = '/assets/hero-panorama-v1.css';
const WIDTHS = [480, 640, 768, 960, 1280, 1600, WIDTH];
const imageUrl = width => `/.netlify/images?url=${HERO}&amp;w=${width}&amp;q=90`;

export const CSS = `
/* Solo el hero: no modifica cabecera, servicios, pueblos ni sus estilos. */
.hero.ac-hero-full{width:100%;max-width:none;margin:0;padding:0;background:#f4f8fb}
...
.hero.ac-hero-full .copy,.hero.ac-hero-full .hero-copy{grid-area:1/1;position:relative;z-index:1;align-self:center;justify-self:start;box-sizing:border-box;width:min(43%,560px);max-width:none;min-width:0;margin:24px clamp(24px,4vw,80px);padding:24px;background:#fff;border:1px solid #dce5ec;border-radius:14px;box-shadow:0 8px 24px rgba(8,43,77,.12);overflow-wrap:anywhere}
...
export function transformHero(html) {
  ...
  for (const [name, value] of Object.entries({
    src: imageUrl(1280),
    srcset: WIDTHS.map(w => `${imageUrl(w)} ${w}w`).join(', '),
  ...
  const oldHrefs = [...original.matchAll(/href="([^"]+)"/g)].map(m => m[1]);
  const newHrefs = [...replacement.matchAll(/href="([^"]+)"/g)].map(m => m[1]);
  if (JSON.stringify(oldHrefs) !== JSON.stringify(newHrefs)) throw new Error('Hero: el cambio alteraría los enlaces');
...
export function validateAsset(file) {
  ...
  if (digest !== HERO_SHA256) throw new Error('Hero: la imagen no coincide con el archivo aprobado; no se publicará');
  return data.length;
}

export function main(root = 'public', dataPath = 'src/localidades.json') {
  const asset = path.join(root, HERO.slice(1));
  // Permite preparar el código sin romper producción mientras se sube la imagen.
  ...
  // Primero se validan todas las páginas; después se escriben los resultados.
  ...
  console.log(`HERO OK: ${results.length} páginas, imagen ${WIDTH}x${HEIGHT}, ${bytes} bytes completos, ancho 100%, textos y enlaces intactos.`);
