...
function replaceFirstParagraph(sectionHtml,text){
  return sectionHtml.replace(/<p>([\s\S]*?)<\/p>/i,`<p>${esc(text)}</p>`);
}
function rewriteSection(html,id,text){
...
for(const d of localidades){
  ...
  html=rewriteSection(html,'parabolicas',pick(copy.parabolicas,key,'parabolicas')(d.localidad));
  html=rewriteSection(html,'porteros',pick(copy.porteros,key,'porteros')(d.localidad));
  html=rewriteSection(html,'telefonia-movil',pick(copy.movil,key,'movil')(d.localidad));
  html=rewriteSection(html,'reparaciones-electricas',pick(copy.electricidad,key,'electricidad')(d.localidad));
