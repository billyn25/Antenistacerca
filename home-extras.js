...
if(!html.includes('<h3>Antenas colectivas y comunidades</h3>')){
  const re=/(<div class="service-grid">)([\s\S]*?)(<\/div>\s*<div class="brands-strip">)/i;
  if(!re.test(html)) throw new Error('Home extras: no encuentro el contenedor service-grid de portada');
  html=html.replace(re,(_,open,items,close)=>open+items+extraMainServices+close);
}
