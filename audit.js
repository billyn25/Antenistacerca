...
for(const f of html){
  ...
  if(canon[0]){
    ...
    if(seenCanonical.has(canon[0]))errors.push(`${rel}: canonical duplicado con ${seenCanonical.get(canon[0])}`);else seenCanonical.set(canon[0],rel);
  }
  ...
  for(const m of s.matchAll(/<img\b[^>]*>/gi)){
    if(!/\balt="[^"]*"/i.test(m[0]))errors.push(`${rel}: imagen sin atributo alt`);
  }

  for(const m of s.matchAll(/<script\s+type="application\/ld\+json">([\s\S]*?)<\/script>/gi)){
    try{JSON.parse(m[1])}catch{errors.push(`${rel}: JSON-LD inválido`)}
  }

  const ids=[...s.matchAll(/\bid="([^"]+)"/gi)].map(m=>m[1]);
  const seenIds=new Set();for(const id of ids){if(seenIds.has(id))errors.push(`${rel}: id duplicado #${id}`);seenIds.add(id)}
...
if(!fs.existsSync(sitemapPath))errors.push('sitemap.xml ausente');
  ...
  for(const c of canonicals)if(!urls.includes(c))errors.push(`sitemap.xml: falta canonical ${c}`);
  for(const u of urls)if(!seenCanonical.has(u))warnings.push(`sitemap.xml: URL sin canonical HTML detectado ${u}`);
}
...
if(!fs.existsSync(robotsPath))errors.push('robots.txt ausente');
  ...
  if(TEST_MODE&&!/Disallow:\s*\//i.test(robots))errors.push('robots.txt: durante pruebas debe bloquear rastreo');
  if(!TEST_MODE&&/Disallow:\s*\/$/im.test(robots))errors.push('robots.txt: sigue bloqueando todo en producción');
  if(!robots.includes(`${DOMAIN}/sitemap.xml`))errors.push('robots.txt: falta referencia al sitemap');
}
