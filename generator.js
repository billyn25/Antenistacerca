...
function province(p,slug,items){
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow"><title>Antenistas en ${esc(p)} | Antenista Cerca</title><meta name="description" content="Técnicos de antenas en municipios de ${esc(p)} para TDT, parabólicas, amplificación, porteros automáticos, cobertura móvil y reparaciones eléctricas."><link rel="canonical" href="${DOMAIN}/${slug}/"><style>:root{--b:#0a3f73;--l:#dce5ec;--r:#d92128;--g:#10ad3e}*{box-sizing:border-box}body{font:16px Arial;margin:0;color:#10243a}.w{max-width:1180px;margin:auto;padding:34px 20px}a{color:var(--b);text-decoration:none}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(190px,1fr));gap:11px;margin-top:22px}.grid a{padding:13px 14px;border:1px solid var(--l);border-radius:10px;font-weight:700}h1{color:var(--b);font-size:40px}.lead{color:#536373;max-width:920px;line-height:1.65}.mobilebar{display:none}@media(max-width:640px){body{padding-bottom:62px}.mobilebar{display:grid;grid-template-columns:1fr 1fr;position:fixed;bottom:0;left:0;right:0;z-index:80}.mobilebar a{padding:15px;text-align:center;color:#fff!important;font-weight:800;background:var(--r)}.mobilebar a:last-child{background:var(--g)}}</style></head><body><main class="w"><p><a href="/">← Inicio</a></p><h1>Técnicos de antenas en ${esc(p)}</h1><p class="lead">Técnico de antenas y antenista para reparación e instalación de antenas de TV y TDT, amplificadores y cabeceras, porteros automáticos y videoporteros, soluciones de cobertura móvil y reparaciones eléctricas del hogar. Trato directo con el técnico desde el primer contacto. Consulta a continuación los municipios en los que prestamos servicio.</p><div class="grid">${links}</div></main><div class="mobilebar"><a href="tel:${TEL}">☎ ${PHONE}</a><a href="https://wa.me/${WA}">WhatsApp</a></div></body></html>`;
}
