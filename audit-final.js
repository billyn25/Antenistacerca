...
import { auditConsent, JS_PATH, CSS_PATH, RUNTIME, CSS } from './cookie-consent.mjs';
...
for(const file of htmlFiles){
  ...
  for(const error of auditConsent(h)) errors.push(`${rel}: ${error}`);
...
// Check the actual published assets, not merely a GA identifier in the HTML.
for(const [url,expected] of [[JS_PATH,RUNTIME],[CSS_PATH,CSS]]){
  const file=path.join(ROOT,url.slice(1));
  if(!fs.existsSync(file)||fs.readFileSync(file,'utf8')!==expected) errors.push(`Consentimiento: recurso ausente o alterado ${url}`);
}
...
console.log(`AUDITORÍA FINAL OK: ${htmlFiles.length} páginas; dominio, canonical, sitemap, robots, consentimiento previo GA4, contactos e índices provinciales validados.`);
